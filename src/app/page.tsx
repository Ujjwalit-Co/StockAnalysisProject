...
export default function StockComparisonDashboard() {
  ...
  const buildStockMatrix = (stockList: Stock[]) => {
    ...
    stockList.forEach((stock) => {
      ...
      const sortedHistorical =
        stock.historical?.sort(
      ...
      for (let i = 0; i < sortedHistorical.length; i++) {
        ...
        const date = new Date(currentPriceData.date)
          .toISOString()
          .split("T")[0];

        const change = previousPriceData
          ? currentPriceData.close - previousPriceData.close
          : 0;
        const changePercent =
          previousPriceData && previousPriceData.close > 0
            ? ((currentPriceData.close - previousPriceData.close) /
                previousPriceData.close) *
              100
            : 0;
  ...
  const searchStocks = async (query: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(
          errorData.error || "Failed to search stocks. Please try again."
        );
        setSearchResults([]);
      }
    } catch (error) {
  ...
  const filteredStocks = stocks
    .filter((stock) => {
      ...
      const matchesPrice = minPrice
        ? (stock.currentPrice || 0) >= parseFloat(minPrice)
        : true;
  ...
  // Exporting from UI
  const exportUIToExcel = () => {
  // Convert your UI matrix (React state) to exportable rows
  const rows = filteredStocks.map(stock => {
    ...
    dates.forEach(date => {
      const data = stockMatrix[stock.symbol]?.[date];
      row[date] = data ? data.price : "-";
    });

    return row;
  });

  // Build sheet headers: Symbol + Company + all dates
  const headers = ["Symbol", "Company", ...dates];
  ...
  saveAs(file, `UI_Stock_Matrix_${new Date().toISOString().split("T")[0]}.xlsx`);
};
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground">
              Real-time stock prices with Last Trading Price (LTP) in Indian
              Rupees
            </p>
              ...
              <DialogContent className="max-w-2xl">
                ...
                <div className="space-y-4">
                  ...
                  {isSearching ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm mt-1">
                        Try a different search term or check the symbol format
                      </p>
                    </div>
                  ) : null}
                  ...
                  <Input
                    ...
                    onKeyPress={(e) =>
                      e.key === "Enter" && handleAddStock(newStockSymbol)
                    }
                  />
            ...
            <CardContent>
              {(() => {
                const topGainer = stocks.reduce(
                  (max, stock) =>
                    (stock.changePercent || 0) > (max?.changePercent || 0)
                      ? stock
                      : max,
                  stocks[0]
                );
            ...
            <CardContent>
              {(() => {
                const topLoser = stocks.reduce(
                  (min, stock) =>
                    (stock.changePercent || 0) < (min?.changePercent || 0)
                      ? stock
                      : min,
                  stocks[0]
                );
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="text-base">
                    Last Trading Prices for your watchlist stocks. Compare
                    performance across dates.
                  </CardDescription>
                </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b-2">
                      <TableHead className="w-20 font-semibold sticky left-0 bg-background z-10">
                        Symbol
                      </TableHead>
                      <TableHead className="w-40 font-semibold sticky left-20 bg-background z-10">
                        Company
                      </TableHead>
                      {dates.map((date) => (
                        <TableHead
                        ...
                        {dates.map((date) => {
                          ...
                          return (
                            ...
                            >
                              {stockData ? (
                                <div
                                  className={`p-2 rounded-md ${getCellBg(
                                    stockData.change
                                  )} border`}
                                >
                                  <div
                                    className={`font-mono text-sm font-semibold ${getCellColor(
                                      stockData.change
                                    )}`}
                                  >
                                  ...
                                  <div
                                    className={`text-xs mt-1 ${getCellColor(
                                      stockData.change
                                    )}`}
                                  >
                                    ...
                                    <br />({stockData.changePercent?.toFixed(2)}
                                    %)
                                  </div>
                                </div>
                              ) : (
                ...
                <div className="text-center py-16 px-4">
                  <div className="max-w-md mx-auto">
                    ...
                    <p className="text-muted-foreground mb-6 leading-relaxed">
                      Your watchlist is empty. Add some Indian stocks to track
                      their Last Trading Prices and compare performance across
                      trading days.
                    </p>
                    <div className="space-y-3">
                      <Button
          ...
          <CardContent className="p-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Download historical price data as CSV or Excel. Uses stored
                  data for efficiency and reliability.
                </p>
              </div>
